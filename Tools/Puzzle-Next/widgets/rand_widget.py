...
import tools.rand as rand

__EMPTY__ = {"entries": []}


class RandTileWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._rand_data = None
        ...
        self.headers = ["Tile Number", "Normal Type", "Special Type", "Random Tile Values"]
    ...
    def _load_into_table(self, data):
        ...
        for rowIndex, entry in enumerate(entries):
            ...
            normaltype = entry.get("normaltype", 0)
            specialtype = entry.get("specialtype", 0)
            ...
            self._setTableItem(rowIndex, 1, str(normaltype))
            self._setTableItem(rowIndex, 2, str(specialtype))
            self._setTableItem(rowIndex, 3, str(valStr))
    ...
    def updateCurrentData(self):
        ...
        for rowIndex in range(rowCount):
            ...
            normaltype  = str(textOrZero(rowIndex, 1))
            specialtype = str(textOrZero(rowIndex, 2))
            values = re.findall(r'0x\w+', textOrZero(rowIndex, 3))
    ...
    def to_bytes(self) -> bytes | None:
        ...
        self._rand_data = rand.encode(self.currentData)
    ...
    def load_from_bin(self, data: bytes | None):
        ...
        self.currentData = rand.decode(io.BytesIO(data))
    ...
    def addRow(self):
        ...
        self._setTableItem(rowCount, 1, "None")  # normaltype
        self._setTableItem(rowCount, 2, "None")  # specialtype
        self._setTableItem(rowCount, 3, "0x0")  # values
    ...
    def loadBinFile(self):
        ...
        self._load_into_table(rand.decode(open(path, 'rb')))
    ...
    def saveBinFile(self):
        ...
        try:
            with open(path, "wb") as f:
                f.write(rand.encode(self.currentData))
