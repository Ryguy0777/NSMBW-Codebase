...
void loadInfo(dModelPlayDate_c *this_) {
	...
	if (level) {
		...
		worldName = getWorldNumber(level->mDisplayWorld);
		levelName = getLevelNumber(level->mDisplayLevel);
		...
		if (level->mDisplayLevel > 19) {
			...
			this_->mpTextBoxes[dModelPlayDate_c::T_pictureFont_00]->SetString(levelName, 0);
		} else {
			...
			this_->mpTextBoxes[dModelPlayDate_c::T_worldCourse_00]->SetString(levelName, 0);
		}
	} else {
		this_->mpTextBoxes[dModelPlayDate_c::T_worldNum_00]->SetString(getWorldNumber(0), 0);
		this_->mpTextBoxes[dModelPlayDate_c::T_pictureFont_00]->SetVisible(false);
		this_->mpTextBoxes[dModelPlayDate_c::T_worldCourse_00]->SetVisible(true);
		this_->mpTextBoxes[dModelPlayDate_c::T_worldCourse_00]->SetString(getWorldNumber(0), 0);
	}
}
#endif